...
void Util::summary(string logfile, int world_size, double acc, double time, double alpha, double error, int effective_epochs, double resultant_acc, double resultant_cost) {
    ofstream myfile(logfile, ios::out | ios::app);
    string timestamp = getTimestamp();
    if (myfile.is_open()) {

        myfile << world_size << "," << time << "," << acc << "," << alpha << "," << "," << error << "," << timestamp << "," << effective_epochs << "," << resultant_acc << "," << resultant_cost << "\n";
